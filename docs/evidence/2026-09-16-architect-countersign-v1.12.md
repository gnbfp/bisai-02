...
| ⑤ 新增回归 15 条 / `pytest` 436 | 开发自评 | **实测 17 条 / 438 passed**（15 / 436 是更早那版的正确读数，**不是错报**） | §4 |
...
|---|---|---|---|
| 1 | `tests\test_migrate_workspace.py` **15 条** | **17 条**（`--collect-only` 实测） | **两个数都对、只是时点不同**：15 条是补上后两条用例**之前**那版的读数；漂点是同一批次里两个时点的数并存，材料按**当前 17** 统一 |
| 2 | `pytest` **436 passed** | **438 passed**（基线 421 + 17） | 同上：436 = 421 + 15（时点早），438 = 421 + 17（时点晚）。**不是错报，也不是环境假红** |

**环境注记（复跑必看）**：**与上面两个数无关** —— 436 / 438 都是在可写 basetemp 下跑出的正常读数；这个坑只影响**没设 basetemp 的复跑**。本机 `%TEMP%\pytest-of-617` 被拒（`PermissionError: [WinError 5]`）⇒ 直接 `python -m pytest` 会有 95 个 setup error 的**假红**。复跑请显式指定可写的 basetemp：
