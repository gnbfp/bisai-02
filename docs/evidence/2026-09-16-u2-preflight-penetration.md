...
### 2.3 run5（19:21:25 – 19:29:23）

原始文件 `%TEMP%\u2_penetration_20260916-192125.log`（20 行）。这一段就是 **④ 非成员半格**取证的那个窗口
（此时花名册已被临时摘成 2 人：神、戚相宜；雨鑫 左不在名册里）。下面 = 第 10–20 行逐字原文。

```
[M0] 2026-09-16T19:26:52 recv id=om_x100b6593810700acc38e7be330ba7e4 chat=oc_2d805f19bf755763edd6c63b835f01df from=ou_a79861323d0c6f45676d3f62b68284e3 type=text text=@_user_1 方向
[M0] 2026-09-16T19:26:53 -> chat_id:oc_2d805f19bf755763edd6c63b835f01df ok | 收到，按评分点想几个候选方向，大概半分钟。
[M0] 2026-09-16T19:26:56 -> chat_id:oc_2d805f19bf755763edd6c63b835f01df ok | 候选方向（仅供参考，由全组拍板）：
1. 为校园周边奶茶店制定营销计划方案
  
[M0] 2026-09-16T19:27:06 recv id=om_x100b65939e6868a8c3a971ca55fb7dc chat=oc_2d805f19bf755763edd6c63b835f01df from=ou_a79861323d0c6f45676d3f62b68284e3 type=text text=1
[M0] 2026-09-16T19:27:24 recv id=om_x100b65939f06f8a8b042a5159fabe84 chat=oc_2d805f19bf755763edd6c63b835f01df from=ou_b8f6fdba44c4bdaa6cb1775c55f5f06f type=text text=1
[M0] 2026-09-16T19:27:25 -> chat_id:oc_2d805f19bf755763edd6c63b835f01df ok | 记下了，你投的是 1. 为校园周边奶茶店制定营销计划方案。想改再回一次数字。
[M0] 2026-09-16T19:27:25 -> chat_id:oc_2d805f19bf755763edd6c63b835f01df ok | 方向定了：1. 为校园周边奶茶店制定营销计划方案，过半：1/1 票。
[升级版] 进程已退出（exit=-1）。
[launcher] child exited 2026-09-16T19:29:23
```
...
| ④ | **非成员**账号群内裸数字 | 静默 | run5：`19:26:52` 雨鑫左 `@方向` 开窗 → `19:27:06` **雨鑫左**（已摘出花名册）不 @ 发 `1`，**无 `->` 行** ⇒ 静默；`19:27:24` **神**（在册成员）不 @ 发 `1` → `19:27:25` 计票 + `过半：1/1 票` ⇒ **同值对照**，差异只可能来自花名册 | ✅ |
...
- **④** 两条都过：主路径（封盘后门禁立刻恢复）+ 非成员半格（run5 `19:26–19:27`，同值对照，见 §2.3 / §8）。
...
### 6.2 `-Seconds` 到点自退：复现结论 + 启动器纪律

run4（`12:27:06` 起）名义 `-Seconds 1200`，实际跑了 **4402 秒**（到 `13:40:28`），最后随 WS `keepalive ping timeout` 掉线才退出 ⇒ 当时记的"到点自退不可靠"。**复现结论：机制本身是好的** ——

- **复现**：`run-upgrade.ps1 -Seconds 60`，`19:30:22` 起、`19:31:23` 退（`exit=0`），`data-upgrade\app.lock` 被删（干净退出会删锁）。
- **机制**（`src\gateway\app.py`）：`if args.seconds > 0: threading.Timer(args.seconds, _stop).start()`，
  `_stop = guard.release() + os._exit(0)`；`SingleInstance.release()` 只 `sock.close()` + `unlink()`，**不阻塞**。
- **那 run4 为什么超时**：`run-upgrade.ps1` **只在 `$Seconds -gt 0` 时**才往 python 传 `--seconds`；不传（或传 0）= **一直跑**。
  run4 用的是当时手搓的启动器，那一版**没有把 `-Seconds` 显式带下去**（该文件后来被覆盖，只能定性到"参数没传到"，无法回溯那一版原文）。
- **启动器纪律**：① 起实例照 `REHEARSAL-0918.md` §0 的现成命令，别手搓；② 手搓就必须把 `-Seconds` **显式**传下去；
  ③ 启动器**首行日志写 `seconds=<N>`**（本轮 probe 就是这么写的，所以能一眼对上；run4 那版没写，只能靠时间戳反推）；
  ④ `-Seconds 0` / 缺省 = **一直跑**，收工必须**显式杀进程** —— `OPERATIONS-U2.md` §1.1 的迁移前置就是"进程已停"，
  `tools\migrate_workspace.py` 的 `check_process_lock()` 会拒绝在跑的实例。
...
| ③ | 19:21:01 | 摘前（基线） | `D9A0E915…` | 3 |
| ③ | 19:21:01 | 摘后 | `D88C7ECE94B141344911BF30E7969AC69102D649F1FF486165210E1B2A3A3157` | 2 |
| ③ | 19:29 后 | 还原后 | `D9A0E915…`（与基线一致） | 3 |

第 ① 轮是手写脚本改的；第 ②③ 轮起用 `D:\rehearsal-logs\roster_toggle.ps1`（`-Drop` / `-Restore`）⇒ ②③ 的"摘后"哈希彼此相同、与第 ① 轮不同（同一份逻辑内容、不同序列化），**但三轮都精确还原回同一个基线哈希 `D9A0E915…`**。
