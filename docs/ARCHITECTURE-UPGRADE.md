...
| 21 | `登记` 的 `≥2` 闸**改判 `others`**（非组长、非机器人的 distinct）+ 一律过滤 `is_bot`；**阈值仍是 `others ≥ 2`、文案不动** | **PM 2026-09-17：收；阈值同日裁定（`others ≥ 2`、文案不动）** —— 落 §9.1 第 19 条 + 回归 1 条；真机翻车佐证见 §9.1 第 19 条。「2 人小组登记不上」是**既有**拒法（真机 `10:48:43` 已拒），非本次引入；放宽 `others ≥ 1` 的待办见 §12.4 |
