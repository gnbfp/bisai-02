...
**⚠ 待裁：PM 2026-09-17 的口述与实现有两处不一致** —— ① 口述写 `《{course}} {title}`，实现是 **`《{title}》 {course}`**；② 口述说 `title` 空时「**不套书名号**」，实现与单测都是**照套**在兜底文件名上（`《指导书》 未标注｜…`）。本行按**代码 + 现有单测**登记；若以口述为准，要改的是**代码与那条单测**，不是本行。
...
| **9/18 彩排前（v1.33 排入；v1.36 两条代码都已落地）** | ① **入群欢迎语 + 注册 `im.chat.member.bot.added_v1`**（PM 2026-09-17 收；级别 = **演示第 1 步阻塞**）—— **已落地（v1.36，`69596bd`）**；② **`登记` 的 `≥2` 闸改判 `others` + 过滤 `is_bot`**（§9.1 第 19 条）—— **已落地（v1.36，`5431690`）**。**执行人 = 开发**。**剩下的只有真机验收** = 独立测试群一句欢迎语 + 一条「组员行含组长」的 `登记`（改造后应被拒），归 9/18 彩排。 |
