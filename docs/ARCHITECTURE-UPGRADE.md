...
| 类 | 时机 | 现状代码位置 |
|---|---|---|
| 入群欢迎（仅一次） | 机器人进群 | **未落地（v1.33 登记）** —— 事件 `im.chat.member.bot.added_v1` **没有任何处理器**：`src/gateway/client.py` 的 `FeishuClient.start()` 只注册了 `register_p2_im_message_receive_v1` ⇒ lark_oapi 直接丢弃该事件（真机 2026-09-17 `10:43:00` / `10:43:17` / `10:48:17` 三次 `processor not found`，日志 `demo-20260917-095737.out.log`）。级别 = **演示第 1 步阻塞**；落点见 §13 与 §12.4 |
...
| 19 | `登记` 组员行里 @ 到**组长自己**或**机器人** | 门禁按**原始 @ 条数**算（`register._collect()` 的 `len(members) < 2`），落盘按**去重后的 `others`** 算 ⇒ 判据与落盘**不同源** | 判据改成 **`others`（非组长、非机器人的 distinct）**：不足 2 人 ⇒ 回 `REGISTER_NEED_MEMBERS`、**不落盘**；`is_bot` 一律先过滤（`events.Mention.is_bot` 早就有） | 沿用「组员那一行至少 2 个人，改完重发一次」；**不说「你 @ 错了」** |

**register `≥2` 闸的真机翻车佐证（v1.33）**：2026-09-17 `10:48:58`（日志 `demo-20260917-095737.out.log`）—— 同一条消息里 `组长：@_user_1` + `组员：@_user_2@_user_3`。**同一个 open_id 在一条消息里 @ 两次会拿到两个不同的占位符** ⇒ 门禁按**原始 2 条**放行，`others` **去重后只剩 1 个组员**：机器人自己的回执写着「组员：勉强算个人（含组长共 2 人）」，而盘上 `members.json` 只有 1 个组员。**根因 = 判据与落盘不同源**，不是「人 @ 少了」。**副作用（v1.33 登记，需 PM 知悉）**：阈值仍是 2，只把判据换成 `others` ⇒ 组员行必须真的 @ 到 **≥2 个不同的人**（组长与机器人不算）；**2 人小组（组长 + 1 名组员）从此登记不上**。真机佐证：2026-09-17 那次落盘的花名册正是「神 + 勉强算个人」= **1 名组员**（旧判据放行）。⇒ 若演示群只有 2 个真人账号，**修完之后演示第 2 步会卡住**：要么先补一个真人账号，要么把阈值降到 `others ≥ 1`（**待 PM 裁**）。现有 `tests/test_gateway_register.py::test_collect_needs_at_least_two_members` 只盖「只 @ 1 个」这一格，**盖不住本格**。
...
- [ ] **19 条**逐条的话术样例（须过 §11 的"说人话"口径）—— **机械判据已落单测（v1.9）**：`tests/test_replies.py` 钉住禁用结构 / 行数预算 / 事实槽位 / 清单派生；**逐条 before-after 与 3 人盲评仍归 9/18**
...
| # | 谁 | 在哪 | 发什么 | 期望看到 | 时长 |
|---|---|---|---|---|---|
| 1 | 组长 | 群 | 把机器人拉进群 | 欢迎语一次（含 5 条高频指令 + 有事 @我）**—— v1.33：未落地（演示第 1 步阻塞），见 §13** | 10s |
...
| 20 | **入群欢迎语**（冷启动一次：一句欢迎 + 5 条高频指令 + 有事 @我）并注册 `im.chat.member.bot.added_v1` | **PM 2026-09-17：收** —— 级别 = **演示第 1 步阻塞**（不挡别的功能，挡的是演示开场那 10 秒）；排 9/18 彩排前（§12.4） |
| 21 | `登记` 的 `≥2` 闸**改判 `others`**（非组长、非机器人的 distinct）+ 一律过滤 `is_bot` | **PM 2026-09-17：收** —— 落 §9.1 第 19 条 + 回归 1 条；真机翻车佐证见 §9.1 第 19 条 |
| 22 | M0 日志补 `mentions=`（原始 key→open_id 映射；无 @ 记 `-`） | **PM 2026-09-17：收**（不涉口径、直接做）—— **已落地（`c35267d`）**，落点见 §13 |
...
| **9/18 彩排前（v1.33 排入）** | ① **入群欢迎语 + 注册 `im.chat.member.bot.added_v1`**（PM 2026-09-17 收；级别 = **演示第 1 步阻塞**）—— 不注册该事件，机器人被拉进群**一声不吭**（真机 3 次 `processor not found`）；注册它同时消掉那行 ERROR。② **`登记` 的 `≥2` 闸改判 `others` + 过滤 `is_bot`**（§9.1 第 19 条）—— 登记是演示第 2 步，真机已翻过车。**执行人 = 开发**（两条都是代码）；**验收证据** = 独立测试群真机一句欢迎语 + 一条「组员行含组长」的 `登记` 回归。**② 的档期是架构师建议，以 PM 为准。** |
...
- [ ] **真机实测**（§4.6 的 ≥6 条穿透 + §9.3 的 19 条话术样例）—— **9/18 彩排**；单测替代不了"平台上 @ 到底长什么样"这条证据
...
| 9 | 架构师 | **已填**（失败路径 **19** 条 = 12 + U4/U6 补 6 条 + v1.33 第 19 条；**三条**纪律）；**v1.9：U6 第 18 条已落地**；**v1.32：U4 第 13–17 条已落地** —— 文案名与测试名落在 §9.1 表后那段；**§9.2 补第三条纪律：查不到人就不编人名（回 `open_id`）**；话术机械判据有单测（`tests/test_replies.py`），18 条样例与真机实测留 9/18 |
...
| 归属注记（`c35267d`） | 架构师（提交方）+ 开发（内容方） | **`c35267d` 的 `src\gateway\app.py` 还带了开发 `804d1dd` 的 3 行接线**（`check_meta_fields` 的 import + 两处调用），被架构师的临时索引一并带入 —— 内容完整、**无丢失，只是归属**。与其他归属注记同款处置：**留痕、不重写历史**。`c35267d` 自身的正文 = `_mentions_log()` + 回归 1 条 |
| 入群欢迎语 + `im.chat.member.bot.added_v1`（v1.33，PM 2026-09-17 收） | 开发 | **未落地** —— 级别 = **演示第 1 步阻塞**，排 9/18 彩排前（§12.4）。现状 = 事件无处理器（`client.py` 的 `FeishuClient.start()` 只注册 `register_p2_im_message_receive_v1`）+ 真机 3 次 `processor not found`（`demo-20260917-095737.out.log`）。落点 = §4.4 白名单第 1 行 / §10.1 第 1 步 |
| `登记` `≥2` 闸改判 `others` + 过滤 `is_bot`（v1.33，PM 2026-09-17 收） | 开发 | **未落地** —— 口径 = §12.3 第 21 条、失败路径 = §9.1 第 19 条。现状 = `register._collect()` 的 `len(members) < 2`（原始 @ 条数）与落盘的 `others` **不同源**，且 `register.py` 全篇**无 `is_bot`**。**回归 = 1 条**（现有 `test_collect_needs_at_least_two_members` 只盖「只 @ 1 个」） |
| M0 日志 `mentions=`（v1.33） | 架构师 | **已落地（`c35267d`）**：`_mentions_log()`（`src\gateway\app.py`）把**原始 key→open_id** 写进 M0 行（无 @ 记 `-`）；回归 = `tests\test_gateway_app.py::test_the_recv_log_records_the_raw_mention_map`。目的 = 这类「只能靠推、不能直接查」的 @ 问题以后**自己就能自证** |
...
> - 2026-09-17 **v1.33（入群欢迎语登记 · `登记` 闸改判 `others` · `mentions=` 归属注记）**：
>   - ① **入群欢迎语 + `im.chat.member.bot.added_v1` 登记**（PM 2026-09-17 收）：落 → **§13 交付行 + §12.4 排期（排 9/18 彩排前）+ §4.4 白名单第 1 行 + §10.1 第 1 步**；级别按 PM 原话写 **「演示第 1 步阻塞」**（不挡别的功能，挡的是演示开场那 10 秒），不写「阻塞级」。顺带把 `register_p2_im_chat_member_bot_added_v1` 注册上 ⇒ 真机那 3 次 `processor not found` 的 ERROR 一起消掉（一举两得）。
>   - ② **`登记` 的 `≥2` 闸改判 `others` + 过滤 `is_bot`**（PM 2026-09-17 收）：落 → **§12.3 第 21 条 + §9.1 第 19 条 + 回归 1 条**。真机翻车佐证 = 2026-09-17 `10:48:58`（日志 `demo-20260917-095737.out.log`）：**同一个 open_id 在一条消息里 @ 两次会拿到两个不同的占位符** ⇒ 门禁按原始条数放行、落盘只算 `others`（回执自写「含组长共 2 人」，`members.json` 只落 1 个组员）。根因 = **判据与落盘不同源**。
>   - ③ **M0 日志 `mentions=`**（PM 2026-09-17 收）已落地（`c35267d`），落点登记进 §13。理由照 PM 原话：这类「只能靠推、不能直接查」的问题已经出现好几次，加一个字段以后自证。
>   - ④ **归属注记（`c35267d`）**：该笔的 `src\gateway\app.py` 含开发 `804d1dd` 的 3 行接线（`check_meta_fields` 的 import + 两处调用），被架构师的临时索引误带入 —— 内容完整、**只是归属**；与 v1.10 的 `bed2574` 同款处置（留痕、不重写历史）。
>   - ⑤ **仍未落地、不许当通过**：①② 两条都是**待写代码**（① 归开发、排 9/18 彩排前；② 的档期是架构师建议、以 PM 为准）。
>   - ⑥ **第 19 条的副作用已登记**（需 PM 知悉）：阈值仍是 2、只把判据换成 `others` ⇒ 组员行必须真的 @ 到 **≥2 个不同的人**，**2 人小组（组长 + 1 名组员）从此登记不上**。真机佐证：2026-09-17 落盘的花名册正是「神 + 勉强算个人」= 1 名组员（旧判据放行）。⇒ 若演示群只有 2 个真人账号，**修完之后演示第 2 步会卡住**：要么补一个真人账号，要么阈值降为 `others ≥ 1`（**待 PM 裁**）。
>   - ⑦ **顺手订正两处 v1.32 留下的计数漂移**：§9.2 标题「**两条**纪律」→「**三条**」（正文自 v1.32 起就有 3 条 bullet）；§13 第 9 行「失败路径 **18** 条…两条纪律」→「**19** 条…**三条**」。连带把 §9.3 与 §12.5 的「18 条话术样例」→「**19 条**」，并同步 `docs\REHEARSAL-0918.md`（头部 / §2 标题 / 记录要点 / 实做清单 + **新增第 19 行**：「组员行里再 @ 一次组长」应被拒、不落盘）。
